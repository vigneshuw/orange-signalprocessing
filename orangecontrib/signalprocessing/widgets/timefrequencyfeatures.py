...
class TimeFrequencyFeatures(OWWidget):
    name = "Time-Frequency Features"
    description = "Compute energy of Wavelet Packet Decomposition (WPD) coefficients from a time series signal"
    icon = "icons/timefrequencyfeatures.svg"
    priority = 6
    keywords = ["time-frequency domain", "features", "signal processing"]
    ...
    def __init__(self):
        ...
        self.feature_descriptions = {
            "EnergyWPD1": "Energy of WPD Coefficient 1: Energy of the first-level WPD coefficients.",
            "EnergyWPD2": "Energy of WPD Coefficient 2: Energy of the second-level WPD coefficients.",
            "EnergyWPD3": "Energy of WPD Coefficient 3: Energy of the third-level WPD coefficients."
        }
        ...
        if self.time_series:
            ...
            for segment in segments:
                ...
                for feature in self.selected_features:
                    if feature == "EnergyWPD1":
                        coeff = wp['a'].data
                        segment_features.append(np.sum(coeff ** 2))
                    elif feature == "EnergyWPD2":
                        coeff = wp['aa'].data
                        segment_features.append(np.sum(coeff ** 2))
                    elif feature == "EnergyWPD3":
                        coeff = wp['aaa'].data
                        segment_features.append(np.sum(coeff ** 2))
    ...
    def send_report(self):
        self.report_caption("Time-Frequency Features Analysis")


if __name__ == "__main__":
    from Orange.widgets.utils.widgetpreview import WidgetPreview
    WidgetPreview(TimeFrequencyFeatures).run()